...
def eval_bc(config, ckpt_name, save_episode=True, num_rollouts=50):
    ...
    env = make_real_env(node=node, setup_robots=True, setup_base=False) #True for mobile. False when we are out of power for base slate
    try:
        robot_startup(node)
        ...
        with torch.inference_mode():
            ...
            for t in range(max_timesteps):
                ...
                if config['policy_class'] == "ACT":
                    ...
                    if temporal_agg:
                        all_time_actions[[t], t:t+num_queries-BASE_DELAY] = all_actions # added "-BASE_DELAY"
